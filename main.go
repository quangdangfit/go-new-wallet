...
// PrepareTransaction :
func PrepareTransaction(privateKey *ecdsa.PrivateKey, client *ethclient.Client) (*bind.TransactOpts, error) {
	publicKey := privateKey.Public()
	publicKeyECDSA, ok := publicKey.(*ecdsa.PublicKey)
	if !ok {
		return nil, fmt.Errorf("ECAdminAmountPrivateKeyIsInvalid")
		//return nil, errs.New(errs.ECAdminAmountPrivateKeyIsInvalid, "ecdsa.PublicKey")
	}
